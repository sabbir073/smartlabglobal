...
export default function ViewsWrapper({ data, filterItems }: Props) {
  const extractData = (response: any) => {
    const items = response?.data?.data?.data?.views?.items;
    return {
  ...
  return (
    <div className="">
      <ContentContainer>
        <div>
          <div className="flex justify-end">
            <DropdownComponent
              ...
              containerClassName="border-2 min-w-[300px] w-full"
        ...
        {dataItems.length === 0 && (
          <NoDataFound containerClassname="" message="No blogs found." />
        )}
        <div className="flex sm:flex-row flex-col flex-wrap gap-5">
          {dataItems.map((item: any, index: number) => {
            const { date, month, year } = formatDate(
              ...
              "object",
              { shortenMonth: true },
            ) as { date: number; month: string; year: number };

            return (
              <div
                key={index}
                className="lg:w-[calc(100%/3-14px)] sm:w-[calc(100%/2-10px)] w-full  aspect-[387/405] group relative grayscale transition-all duration-500 ease-in-out hover:grayscale-0 mt-16"
              >
                ...
                />
                <div className="w-full h-full absolute top-0 left-0 bg-[#0000008F] -z-[1]"></div>
                <div className="w-full h-full group-hover:backdrop-blur-sm p-[30px]">
                  <div className="">
                    <Heading2 className=" !text-[#F4F4F4] !text-[18px] !mb-6 leading-[1.55] font-[500]">
                      <SafeHtml content={item.title} />
                    </Heading2>
                    <p className="text-[14px] text-[#F4F4F4] mb-10">
                      {month + " " + date + ", " + year}
                    </p>
                    <div className="group-hover:opacity-100 opacity-0 transition-all duration-500  text-[#F4F4F4]">
                      <p className="text-[#D9D9D9] text-sm 2xl:mb-16 mb-8">
...
let data = ["category1", "category2", "category3"];
